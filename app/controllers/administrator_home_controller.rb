# -*- coding: utf-8 -*-
class AdministratorHomeController < ApplicationController
  before_filter :authenticate_administrator!
  
  
  def show_formal_technical_reviews
    result = FormalTechnicalReview.all
    @rtf = Kaminari.paginate_array(result).page(params[:page]).per(5)
    logger.info("RTF......."  + @rtf.inspect)
    ...
    if params.include? :subsystem_name
      ...
      @rtf = FormalTechnicalReview.where(:subsystem_id => tmp).page(params[:page]).per(5)
      logger.info("RTF......."  + @rtf.inspect)
